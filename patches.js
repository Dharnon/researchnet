...
const oldEmpty = `connectedResearchers.length === 0 ? (
...
const newEmpty = `connectedResearchers.length === 0 ? (
            <div style={{ textAlign: "center", padding: "80px 32px", background: t.surface, border: \`1px solid \${t.border}\`, borderRadius: 16 }}>
              ...
              <p style={{ fontSize: 16, fontWeight: 600, color: t.textPrimary, marginBottom: 8 }}>Tu red est\u00e1 vac\u00eda</p>
              <p style={{ fontSize: 13, color: t.textSecondary, maxWidth: 280, margin: "0 auto 28px", lineHeight: 1.6 }}>Explora investigadores y con\u00e9ctate para construir tu red de colaboraci\u00f3n.</p>
              <button onClick={() => setView("discover")} style={{ padding: "10px 22px", borderRadius: 8, background: t.accent, color: "#000", border: "none", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: "0.01em", transition: "box-shadow 0.2s ease" }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 0 20px " + t.accent + "55"; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; }}
              >Explorar investigadores</button>
            </div>`;

if (t.includes(oldEmpty)) {
  t = t.replace(oldEmpty, newEmpty);
  console.log('Replaced empty state');
} else {
  console.log('OLD EMPTY NOT FOUND');
}

// ── 2. Conversation list item upgrade ───────────────────────────────────────
const oldConv = `conversations.map((conv) => (
...
if (t.includes(oldConv)) {
  t = t.replace(oldConv, newConv);
  console.log('Replaced conversations.map');
} else {
  console.log('OLD CONV NOT FOUND');
}
